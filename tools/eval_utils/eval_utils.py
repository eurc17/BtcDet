...
def get_match_stats(ret_dict, metric):
    pos_num, neg_num, total, precision, recall, f1, pos_all_num = (
        ret_dict["pos_num"],
        ret_dict["neg_num"],
        ret_dict["total"],
        ret_dict["precision"],
    ...
    for cur_thresh in range(1, 10):
        metric["total_occ_num_box_%.1f" % (cur_thresh * 0.1)] += occ_box_num[
            cur_thresh - 1
        ]
...
def eval_one_epoch(
    ...
    if hasattr(cfg.MODEL, "OCC") and hasattr(cfg.MODEL.OCC, "OCC_POST_PROCESSING"):
        ...
        logger.info(
            " ".join(
                [
                    f"precision: {metric['precision'] / metric['scene_num']:.3f}, recall: {metric['recall'] / metric['scene_num']:.3f},",
                    f"f1: {metric['f1'] / metric['scene_num']:.3f}, precision_factored: {metric['precision_factored'] / metric['scene_total_factor']:.3f}",
                    f"recall_factored: {metric['recall_factored'] / metric['scene_total_factor']:.3f}, f1_factored: {metric['f1_factored'] / metric['scene_total_factor']:.3f}",
                ]
            )
        )
        logger.info(
            " ".join(
                [
                    f"occ thresh {i * 0.1:.1f}: {metric['total_occ_num_box_%.1f' % (i*0.1)] / metric['total_num_box']:.3f},  "
                    for i in range(1, 10)
                ]
            )
        )
        logger.info(
            " ".join(
                [
                    f" total_pos_all_portion {metric['total_pos_all_portion'] / metric['scene_num']:.3f}"
                ]
            )
        )
