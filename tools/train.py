...
from btcdet.models import build_network, model_fn_decorator
...
def parse_config():
    ...
    parser.add_argument(
        "--output_dir", type=str, default="", help="log location of training"
    )
    parser.add_argument(
        "--pretrained_model", type=str, default=None, help="log location of training"
    )
    parser.add_argument(
        "--pre_occ", type=str, default=None, help="log location of training"
    )
    parser.add_argument(
        "--pre_det", type=str, default=None, help="log location of training"
    )
...
def main():
    ...
    if args.batch_size is None:
        ...
        print(
            "args.batch_size = cfg.OPTIMIZATION.BATCH_SIZE_PER_GPU = ", args.batch_size
        )
    else:
    ...
    print("output_dir should be made *****************************************")
    ...
    if cfg.get("OCC_OPTIMIZATION", None) is not None and cfg.OCC_OPTIMIZATION.LR > 0.0:
        occ_optimizer = build_optimizer(
            model, cfg.OCC_OPTIMIZATION, para_lst_name="occ"
        )
    ...
    if args.pre_occ is not None:
        model.load_params_from_file(
            filename=args.pre_occ, to_cpu=dist, logger=logger, prefix="occ"
        )
    if args.pre_det is not None:
        model.load_params_from_file(
            filename=args.pre_det, to_cpu=dist, logger=logger, prefix="det"
        )

    if args.ckpt is not None:
        it, start_epoch = model.load_params_with_optimizer_lst(
            args.ckpt, to_cpu=dist, optimizer_lst=optimizer_lst, logger=logger
        )
