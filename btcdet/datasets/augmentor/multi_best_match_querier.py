...
class MltBestMatchQuerier(object):
    ...
    def add_gtbox_best_match_points_to_scene(self, data_dict):
        ...
        for idx in range(gt_boxes_num):
            ...
            if gt_name in self.class_names:
                ...
                file_path = self.mlt_bm_root[gt_name] / "{}_{}.pkl".format(
                    image_idx, gt_box_id
                )
                with open(file_path, "rb") as f:
                    obj_points = pickle.load(f)
                obj_points = obj_points.reshape([-1, self.load_point_features])[
                    :, :3
                ].astype(np.float32)
    ...
    def add_sampled_boxes_best_match_points_to_scene(self, data_dict):
        ...
        for ind in range(aug_length):
            ...
            file_path = self.mlt_bm_root[gt_name] / "{}_{}.pkl".format(
                aug_boxes_image_idx[ind], aug_boxes_gt_idx[ind]
            )
            with open(file_path, "rb") as f:
                obj_points = pickle.load(f)
            obj_points = obj_points.reshape([-1, self.load_point_features])[
    ...
    def filter_bm(self, data_dict):
        gt_boxes_filtered = (
            data_dict["gt_boxes"][data_dict["gt_boxes_mask"]]
            if "gt_boxes_mask" in data_dict
            else data_dict["gt_boxes"]
        )

        assert len(gt_boxes_filtered) == len(data_dict["bm_points"]), "{}_{}".format(
            len(data_dict["gt_boxes"]), len(data_dict["bm_points"])
        )
        boxes_xy_dist = np.linalg.norm(gt_boxes_filtered[:, :2], axis=-1)
        box_inds = np.argsort(boxes_xy_dist)
        gt_boxes, bm_points_lst = gt_boxes_filtered[box_inds, :], [
            data_dict["bm_points"][box_inds[i]] for i in range(len(box_inds))
        ]
        box_labels = (
            np.arange(len(gt_boxes_filtered), dtype=np.float32).reshape(-1, 1) + 1
        )
        boxes = np.concatenate([gt_boxes, box_labels], axis=-1)
        pointwise_box_label = np.round(
            point_box_utils.points_in_box_3d_label(
                data_dict["points"][:, :3],
                boxes,
                slack=1.0,
                shift=np.array([[0.0, 0.0, 0.15, 0.0, 0.0, 0.0]]),
            )
        ).astype(np.int)
        ...
        if self.vis:
            other_occluder_mask, miss_tot_mask, bm_tot_mask, other_tot_mask = (
                np.zeros([nx, ny, nz], dtype=np.uint8),
                np.zeros([nx, ny, nz], dtype=np.uint8),
                np.zeros([nx, ny, nz], dtype=np.uint8),
                np.zeros([nx, ny, nz], dtype=np.uint8),
            )
        ...
        for i in range(len(bm_points_lst)):
            obj_points = data_dict["points"][:raw_points_len, :][
                pointwise_box_label == (i + 1), :
            ]
            avg_feature = (
                np.mean(
                    obj_points[:, 3 : self.num_point_features], axis=0, keepdims=True
                )
                if len(obj_points) > 0
                else np.zeros([1, self.num_point_features - 3])
            )
            (
                _,
                obj_sphere_points,
                obj_sphere_coords,
                obj_expand_sphere_coords,
                obj_voxelwise_mask,
                _,
                _,
                _,
            ...
            bm_points = np.concatenate(
                [
                    bm_points_lst[i],
                    np.tile(avg_feature, (bm_points_lst[i].shape[0], 1)),
                ],
                axis=-1,
            )
            ...
            if self.vis:
                miss_mask = (
                    keep_mask * rmv_self_filter_mask * (1 - rmv_miss_filter_mask)
                ).astype(np.bool)
                self_mask = (keep_mask * (1 - rmv_self_filter_mask)).astype(np.bool)
                other_mask = (
                    keep_mask * rmv_miss_filter_mask * rmv_self_filter_mask
                ).astype(np.bool)
                miss_sphere_coords, miss_points = self.get_nearest_points(
                    miss_mask, bm_sphere_points, bm_sphere_coords, bm_points, axis=0
                )
                self_sphere_coords, self_points = self.get_nearest_points(
                    self_mask, bm_sphere_points, bm_sphere_coords, bm_points, axis=0
                )
                other_sphere_coords, other_points = self.get_nearest_points(
                    other_mask, bm_sphere_points, bm_sphere_coords, bm_points, axis=0
                )
                other_bm_points_lst.append(other_points), self_bm_points_lst.append(
                    self_points
                ), miss_bm_points_lst.append(miss_points)
                miss_tot_mask[
                    miss_sphere_coords[..., 0],
                    miss_sphere_coords[..., 1],
                    miss_sphere_coords[..., 2],
                ] = np.ones_like(miss_sphere_coords[..., 0], dtype=np.uint8)
                bm_tot_mask = np.maximum(bm_tot_mask, bm_voxelwise_mask)
                other_tot_mask[
                    other_sphere_coords[..., 0],
                    other_sphere_coords[..., 1],
                    other_sphere_coords[..., 2],
                ] = np.ones_like(other_sphere_coords[..., 0], dtype=np.uint8)
            ...
            bm_sphere_coords, bm_points = self.get_nearest_points(
                keep_mask, bm_sphere_points, bm_sphere_coords, bm_points, axis=0
            )
            if self.expand:
                bm_expand_sphere_coords = self.expand_voxel(
                    bm_sphere_coords, nx, ny, nz
                )
            else:
            ...
            voxelwise_mask[
                bm_expand_sphere_coords[..., 0],
                bm_expand_sphere_coords[..., 1],
                bm_expand_sphere_coords[..., 2],
            ] = np.ones_like(bm_expand_sphere_coords[..., 0], dtype=np.uint8)
            occ_mask = (np.cumsum(voxelwise_mask, axis=0) > 0.5).astype(np.uint8)

            if self.add_bm_2_raw:
                data_dict["points"] = np.concatenate(
                    [data_dict["points"], bm_points], axis=0
                )
        if self.vis:
            self_cum = np.cumsum(bm_tot_mask, axis=0)
            self_reverse_cum = np.flip(
                np.cumsum(np.flip(bm_tot_mask, axis=0), axis=0), axis=0
            )
            self_tot_occ_mask = (self_cum > 0.5).astype(
                np.uint8
            )  # - bm_tot_mask.astype(np.uint8)
            self_limit_occ_mask = (self_reverse_cum > 0.5).astype(
                np.uint8
            ) * self_tot_occ_mask
            ...
            other_occluder_mask[
                ...
                raw_sphere_coords[..., 1],
                raw_sphere_coords[..., 2],
            ] = np.ones_like(raw_sphere_coords[..., 0], dtype=np.uint8)

            other_full_tot_occ_mask = (
                np.cumsum(other_occluder_mask, axis=0) > 0.5
            ).astype(np.uint8) * self.propagate_323(other_tot_mask)
            ...
            data_dict.update(
                {
                    ...
                    "miss_occ_points": self.get_voxel_centers(
                        miss_tot_occ_mask, sphere_coords_min, self.sphere_coords_res
                    ),
                    "miss_full_occ_points": self.get_voxel_centers(
                        miss_full_tot_occ_mask,
                        sphere_coords_min,
                        self.sphere_coords_res,
                    ),
                    "self_occ_points": self.get_voxel_centers(
                        self_tot_occ_mask, sphere_coords_min, self.sphere_coords_res
                    ),
                    "self_limit_occ_mask": self.get_voxel_centers(
                        self_limit_occ_mask, sphere_coords_min, self.sphere_coords_res
                    ),
                    "other_occ_points": self.get_voxel_centers(
                        other_tot_occ_mask, sphere_coords_min, self.sphere_coords_res
                    ),
                    "other_full_occ_points": self.get_voxel_centers(
                        other_full_tot_occ_mask,
                        sphere_coords_min,
                        self.sphere_coords_res,
                    ),
    ...
    def get_voxel_centers(self, mask, sphere_coords_min, sphere_coords_res):
        ...
        return coords_utils.uvd2absxyz_np(
            sphere_points[..., 0],
            sphere_points[..., 1],
            sphere_points[..., 2],
            type="sphere",
    ...
    def exclude_map(self, occ_coords, x_epansion=2):
        ...
        expand_occ_coords = (np.expand_dims(occ_coords, axis=1) + xyz_shift).reshape(
            -1, 3
        )
    ...
    def expand_voxel(self, occ_coords, nx, ny, nz, yz_epansion=2):
        ...
        expand_occ_coords = np.minimum(
            np.maximum(np.array([[0, 0, 0]]), expand_occ_coords),
    ...
    def get_nearest_points(
        self, keep_mask, bm_sphere_points, bm_sphere_coords, bm_points, axis=0
    ):
        ...
        bm_points, bm_sphere_coords = (
            bm_points[closeind, :],
            bm_sphere_coords[closeind, :],
        )
    ...
    def get_coords(
        ...
        if sphere_coords_min is None:
            sphere_coords_min = np.min(
                raw_sphere_points, axis=0, keepdims=True
            ) - np.asarray([[0.2 * 10, 0.0875 * 2 * 10, 0.4203125 * 10]])
        raw_sphere_coords = np.floor_divide(
            raw_sphere_points - sphere_coords_min, self.sphere_coords_res
        ).astype(np.int32)
        unique_occupied = 0
        if nx is None:
        ...
        # print("nx",nx,ny,nz)
        raw_expand_sphere_coords = (
            self.expand_voxel(raw_sphere_coords, nx, ny, nz)
            if (expand and self.expand)
            else raw_sphere_coords
        )
        raw_expand_sphere_coords = (
            self.exclude_map(raw_sphere_coords)
            if x_expand
            else raw_expand_sphere_coords
        )
        voxelwise_mask = np.zeros([nx, ny, nz], dtype=np.uint8)
        voxelwise_mask[
            raw_expand_sphere_coords[..., 0],
            raw_expand_sphere_coords[..., 1],
            raw_expand_sphere_coords[..., 2],
        ] = np.ones_like(raw_expand_sphere_coords[..., 0], dtype=np.uint8)
        return (
            points,
            raw_sphere_points,
            raw_sphere_coords,
    ...
    def __call__(self, data_dict):
        ...
        # aug_boxes_image_idx added in DataBaseSampler, which is called by gt_sampling() in data_augmentor.py
        ...
        if (
            self.querier_cfg.get("ABLATION", None) is not None  # default = None
            and len(data_dict["bm_points"]) > 0
        ):
