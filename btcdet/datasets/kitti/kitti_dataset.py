...
class KittiDataset(DatasetTemplate):
    def __init__(
        ...
        # split_dir contains the frame ids to use, it is actually a txt file
        split_dir = self.root_path / "ImageSets" / (self.split + ".txt")

        # reads out the frame ids contained in the txt, used for training or testing
        self.sample_id_list = (
    ...
    def include_kitti_data(self, mode):
        ...
        if self.frame_id is not None:
            self.kitti_infos = [
                ...
                if info["point_cloud"]["lidar_idx"] == self.frame_id
            ]
    ...
    def create_info_file_with_coverage(self, mlt_bm_root, gt_db_root, num_workers=1):
        ...
        def get_coords(points, nx=None, ny=None, nz=None, sphere_coords_min=None):
            ...
            if sphere_coords_min is None:
                sphere_coords_min = np.minimum(
                    np.min(raw_sphere_points, axis=0, keepdims=False),
                    np.array([0.0, 0.0, 0.0]),
                ).reshape(1, 3)
            raw_sphere_coords = np.floor_divide(
                raw_sphere_points - sphere_coords_min, sphere_coords_res
            ).astype(np.int32)
        ...
        def process_single_info(info):
            ...
            for i in range(len(annotations["name"])):
                if annotations["name"][i] not in ["Car", "Pedestrian", "Cyclist"]:
                    ...
                    bm_file_path = mlt_bm_root[
                        annotations["name"][i]
                    ] / "{}_{}.pkl".format(int(image_idx), i)
                    gt_db_path = gt_db_root / "{}_{}_{}.bin".format(
                        image_idx, annotations["name"][i], i
                    )
                    with open(bm_file_path, "rb") as f:
                        bm_points = (
                            pickle.load(f).reshape([-1, 3])[:, :3].astype(np.float32)
                        )
                    obj_points = np.fromfile(str(gt_db_path), dtype=np.float32).reshape(
                        [-1, 4]
                    )[:, :3] + gt_box[:3].reshape(-1, 3)
                    gtrotation = point_box_utils.get_yaw_rotation(gt_box[6])
                    bm_points = np.einsum("nj,ij->ni", bm_points, gtrotation) + gt_box[
                        :3
                    ].reshape(-1, 3)
                    sphere_coords_min, nx, ny, nz, num_unique_bm = get_coords(
                        bm_points, nx=None, ny=None, nz=None, sphere_coords_min=None
                    )
                    _, _, _, _, num_unique_obj = get_coords(
                        obj_points,
                        nx=nx,
                        ny=ny,
                        nz=nz,
                    ...
                    print(
                        annotations["name"][i],
                        num_unique_obj / max(1, num_unique_bm),
                        num_unique_obj,
                        num_unique_bm,
                    )
            ...
            if len(coverage_rate_lst) == 0:
                ...
                info["annos"]["coverage_rates"] = np.asarray(
                    coverage_rate_lst[0]
                ).reshape(-1, 1)
            else:
    ...
    def evaluation(self, det_annos, class_names, **kwargs):
        ...
        ap_result_str, ap_dict, pcrc_dict = kitti_eval.get_official_eval_result(
            eval_gt_annos,
            eval_det_annos,
            class_names,
            coverage_rates=kwargs["coverage_rates"],
        )
...
def create_kitti_infos_with_coverage(
    ...
    mlt_bm_root = {
        ...
        "Pedestrian": save_path
        / "bm_5maxdist_1num_Pedestrian",  # bm_5maxdist_2num_Pedestrian
    }
    ...
    train_coveraged_infos = dataset.create_info_file_with_coverage(
        mlt_bm_root, train_gt_root, num_workers=workers
    )
    ...
    val_coveraged_infos = dataset.create_info_file_with_coverage(
        mlt_bm_root, val_gt_root, num_workers=workers
    )
