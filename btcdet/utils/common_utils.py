...
def sparse_trilinear_interpolate_torch(features, binds, idxs, grid_size):
    ...
    weights = torch.stack(
        [w000, w010, w001, w011, w100, w110, w101, w111], dim=-1
    ).unsqueeze(
        -1
    )  # N, 8, 1
    x0, y0, z0, x1, y1, z1 = (
    ...
    idx_mask = torch.all(idxs >= 0, dim=-1) & torch.all(
        idxs < torch.as_tensor(grid_size, device=idxs.device).unsqueeze(0), dim=-1
    )
    features, bidxs = features[idx_mask], bidxs[idx_mask]
    unique_idxs, reverse_idxs, labels_count = bidxs.unique(
        dim=0, return_inverse=True, return_counts=True
    )
    # print(binds.dtype, bidxs.dtype, unique_idxs.dtype, reverse_idxs.dtype)
    features = torch.zeros(
        [unique_idxs.shape[0], F], dtype=torch.float32, device=features.device
    ).index_add_(0, reverse_idxs.long(), features)
