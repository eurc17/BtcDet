...
def cartesian_cylinder_coords(cartesian_points, perm="xyz"):
    ...
    cylin_x = xydist
    cylin_y = torch.atan2(-y, x) * (180.0 / np.pi)
    cylin_z = z
...
def absxyz_2_spherexyz_np(points):
    ...
    sphere_x = dist
    sphere_y = np.arctan2(-y, x) * 180.0 / np.pi
    sphere_z = np.arctan2(z, xydist) * 180.0 / np.pi
...
def absxyz_2_cylinxyz_np(points):
    ...
    cylin_x = xydist
    cylin_y = np.arctan2(-y, x) * 180.0 / np.pi
    cylin_z = z
...
def creat_grid_coords(pntrange, voxel_size):
    ...
    # print("xyz", xyz.shape)
    voxel_centers = (0.5 + xyz.astype(np.float32)) * voxel_size.reshape(
        1, 1, 1, 3
    ) + origin.reshape(1, 1, 1, 3)
