...
class Detector3DTemplate(nn.Module):
    def __init__(self, model_cfg, num_class, dataset, full_config=None):
        ...
        if model_cfg.get("OCC", None) is not None:
            self.occ_num_class = (
                1 if model_cfg.OCC.PARAMS.CLASS_AGNOSTIC else num_class
            )  # true for default config
        else:
    ...
    def create_subvox_loc(self):
        ...
        voxel_size = torch.tensor(
            self.dataset.dataset_cfg.OCC.VOXEL_SIZE, dtype=torch.float32, device="cuda"
        )
        all_voxel_centers = coords_utils.get_all_voxel_centers_zyx(
            1, grids_num, range_origin, voxel_size
        )[
            0, ...
        ...
        return {
            "all_voxel_centers": all_voxel_centers,
            "all_voxel_centers_2d": all_voxel_centers_2d,
        }
    ...
    def build_occ_pnt_update(self, model_info_dict, occ=True):
        ...
        if self.print:
            print("build occ pnt update mode:", self.dataset.mode)
        model_info_dict["occ_module_list"].append(occ_pnt_module)
        if occ_pnt_module.config_rawadd:
            model_info_dict["num_rawpoint_features"] = (
                model_info_dict["num_rawpoint_features"] + occ_pnt_module.code_num_dim
            )
        model_info_dict["num_voxel_point_features"] = (
            model_info_dict["num_voxel_point_features"] + occ_pnt_module.code_num_dim
        )
    ...
    def build_vfe(self, model_info_dict, occ=False):
        ...
        if not occ:
            maxprob = (
                self.dataset.dataset_cfg.OCC.MAX_VFE
                if self.dataset.dataset_cfg.get("OCC", None) is not None
                and self.dataset.dataset_cfg.OCC.get("MAX_VFE", None) is not None
                else False
            )
        vfe_module = vfe.__all__[model_cfg.VFE.NAME](
            ...
            voxel_size=model_info_dict["occ_voxel_size"]
            if occ
            else model_info_dict["det_voxel_size"],
            data_cfg=self.dataset.dataset_cfg,
            grid_size=model_info_dict["occ_grid_size"]
            if occ
            else model_info_dict["det_grid_size"],
    ...
    def build_backbone_3d(self, model_info_dict, occ=False):
        ...
        if occ:
            ...
            model_info_dict[
                "num_occ_3d_features"
            ] = backbone_3d_module.num_point_features
        else:
    ...
    def build_map_to_bev_module(self, model_info_dict, occ=False):
        ...
        model_info_dict[
            "pre_conv_num_bev_features"
        ] = map_to_bev_module.num_bev_features
    ...
    def build_occ_point_head(self, model_info_dict):
        ...
        point_head_module = dense_heads.__all__[self.model_cfg.OCC_POINT_HEAD.NAME](
            ...
            if not self.model_cfg.OCC_POINT_HEAD.CLASS_AGNOSTIC
            else 1,
    ...
    def build_occ_dense_head(self, model_info_dict, occ=True):
        ...
        if model_cfg.get("OCC_DENSE_HEAD", None) is None:
            return None, model_info_dict
        occ_dense_head_module = occ_pnt.occ_dense_heads.__all__[
            model_cfg.OCC_DENSE_HEAD.NAME
        ](
            model_cfg=model_cfg,
            data_cfg=self.dataset.dataset_cfg,
            input_channels=model_info_dict["num_occ_3d_features"]
            if "num_occ_3d_features" in model_info_dict
            else model_info_dict["num_bev_features"],
            num_class=self.occ_num_class,
            grid_size=model_info_dict["occ_grid_size"],
        )
            ...
            record_dict.update(
                {
                    ...
                    "iou": iou3d.cpu().numpy()
                    if iou3d is not None and len(iou3d) == len(final_boxes)
                    else None,
                }
            )
    ...
    def occ_post_processing(self, batch_dict):
        ...
        precision, recall, f1 = self.call_precision_recall_f1(
            pos_num, pos_mask, batch_pred_occ_prob, 0.5
        )
        match_dicts = {
            "pos_num": pos_num.cpu(),
            "neg_num": neg_num.cpu(),
        ...
        if "occ_pnts" in batch_dict:
            batch_size, added_occ_b_ind, occ_pnts, gt_boxes_num, gt_boxes = (
                batch_dict["batch_size"],
                batch_dict["added_occ_b_ind"],
                batch_dict["occ_pnts"],
            ...
            ) = point_box_utils.torch_points_in_box_3d_box_label_batch(
                occ_pnts[..., :3],
                added_occ_b_ind.unsqueeze(-1),
                gt_boxes,
                gt_boxes_num,
                batch_size,
            )
            ...
            match_dicts.update(
                {"box_num_sum": box_num_sum, "occ_box_num": occ_thresh_box_num}
            )
            # print(box_num_sum)
    ...
    def load_params_from_file(
        self, filename, logger, to_cpu=False, verbose=True, prefix=""
    ):
        assert os.path.isfile(filename), "wrong file path: {}".format(filename)

        logger.info(
            "==> Loading %s parameters from checkpoint %s to %s"
            % (prefix, filename, "CPU" if to_cpu else "GPU")
        )
    ...
    def load_params_with_optimizer_lst(
        self, filename, to_cpu=False, optimizer_lst=None, logger=None
    ):
        if not os.path.isfile(filename):
            raise FileNotFoundError
        ...
        if optimizer_lst is not None:
            if (
                "optimizer_state_lst" in checkpoint
                and checkpoint["optimizer_state_lst"] is not None
            ):
                logger.info(
                    "==> Loading optimizer_lst parameters from checkpoint %s to %s"
                    % (filename, "CPU" if to_cpu else "GPU")
                )
                for i in range(len(optimizer_lst)):
                    optimizer_lst[i].load_state_dict(
                        checkpoint["optimizer_state_lst"][i]
                    )
            else:
                assert filename[-4] == ".", filename
                src_file, ext = filename[:-4], filename[-3:]
